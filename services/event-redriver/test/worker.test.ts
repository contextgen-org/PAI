...
describe("event redriver polling shell", () => {
  ...
  it("fails closed when the durable reconciler dependency is absent", () => {
    expect(
      () =>
        new EventRedriverWorkerV1({
  ...
  it("treats a widened or inconsistent batch summary as a worker failure", async () => {
    const widened = new EventRedriverWorkerV1({
      reconciler: {
    ...
    const inconsistent = new EventRedriverWorkerV1({
      reconciler: {
        async reconcileBatch() {
          return {
            claimed: 1,
    ...
    const impossibleProbeAccounting = new EventRedriverWorkerV1({
      reconciler: {
        async reconcileBatch() {
          return {
            claimed: 1,
    ...
    await expect(impossibleProbeAccounting.runOnce()).rejects.toThrow(
      "inconsistent batch summary",
    );
  });
  ...
  it("binds the configured port method and rejects accessor-backed summaries", async () => {
    ...
    reconciler.reconcileBatch = async () => {
      replacementCalls += 1;
      return empty;
    };
    await expect(worker.runOnce()).resolves.toEqual(empty);
    ...
    let getterCalls = 0;
    const accessorSummary = { ...empty } as Record<string, unknown>;
    ...
    const accessorWorker = new EventRedriverWorkerV1({
      reconciler: {
        async reconcileBatch() {
  ...
  it("coalesces duplicate ticks while durable reconciliation is in flight", async () => {
    let release!: () => void;
  ...
  it("turns synchronous port failures into rejections and isolates a throwing reporter", async () => {
    let calls = 0;
    let reports = 0;
    const failure = new Error("synchronous reconciler failure");
    const worker = new EventRedriverWorkerV1({
      reconciler: {
        reconcileBatch() {
          calls += 1;
  ...
  it("installs the single-flight promise before a synchronous re-entry", async () => {
    let worker!: EventRedriverWorkerV1;
    let calls = 0;
    let nested: Promise<unknown> | undefined;
    worker = new EventRedriverWorkerV1({
      reconciler: {
        reconcileBatch() {
          calls += 1;
  ...
  it("linearizes stop against a concurrent start and permits restart after stop", async () => {
    ...
    expect(() => worker.start()).toThrow("event redriver worker is stopping");
  ...
  it("aborts a hanging reconciliation but rejects bounded stop until it really settles", async () => {
    vi.useFakeTimers();
    let release!: () => void;
    const blocked = new Promise<void>((resolve) => { release = resolve; });

...
export interface ActionRuntimeObjectStoreCompositionV1 {
...
export async function openActionRuntimeObjectStoreV1(
  options: ActionRuntimeObjectStoreOptionsV1,
): Promise<ActionRuntimeObjectStoreCompositionV1> {
  if (!identifierPatternV1.test(options.worker_id)) {
    throw new Error("Action Runtime ObjectStore worker_id is invalid");
  }
  ...
  if (
    ...
    throw new Error("Action Runtime ObjectStore interval is invalid");
  }
  ...
  const foregroundPool = new Pool({
    ...
    application_name: "pai_action_runtime_object_store",
    max: 4,
    connectionTimeoutMillis: 5_000,
  });
  const reconcilerPool = new Pool({
    connectionString: options.reconciler_database_url,
    application_name: "pai_action_runtime_object_store_reconciler",
    max: 2,
    connectionTimeoutMillis: 5_000,
  });
  let closed = false;
  ...
  try {
      ...
      policies: [
        {
          ...
          media_types: [
            ...
            "application/gzip",
            "application/vnd.pai.skill-bundle",
            "text/plain",
          ],
          capabilities: {
    ...
    const reconciliation = adapter as ObjectStorePortV1 &
      ObjectStoreReconciliationPortV1;
    ...
    return Object.freeze({
      ...
      start() {
        if (closed) throw new Error("Action Runtime ObjectStore is closed");
        worker.start();
      },
